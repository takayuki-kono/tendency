...
# ===== 探索スキップ設定 =====
# Trueにすると、保存された探索結果を使用（新規探索しない）
# Falseにすると、新規に探索を実行して結果を保存
...
STAGE1_MAX_TRIALS = 10     # 10パターン試行（元: 50）
STAGE1_MAX_EPOCHS = 8      # 最大8エポック（元: 15）
STAGE1_FACTOR = 3          # Hyperband factor（悪い設定を早期打ち切り）

# Stage 2: データ拡張探索（構造は固定）
STAGE2_MAX_TRIALS = 10     # 10パターン試行（元: 50）
STAGE2_MAX_EPOCHS = 8      # 最大8エポック（元: 15）
STAGE2_FACTOR = 3          # Hyperband factor
...
img_size = 224
img_size = 224
BATCH_SIZE = 64  # GPU使用率向上のため32から64へ変更
# =================================================================
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f'efficient_tuner_log_{MODEL_TO_USE}.txt', mode='w')
    ],
    force=True
)
...
class MultiStageHyperModel(kt.HyperModel):
    ...
    def build(self, hp):
        ...
        for i in range(num_dense_layers):
            head = layers.Dense(units=dense_units[i], activation='relu')(head)
...
def main():
    logger.info(f"=== Starting Efficient 2-Stage Tuning for {MODEL_TO_USE} ===")
    logger.info(f"Stage 1: Structure tuning - {STAGE1_MAX_TRIALS} trials, {STAGE1_MAX_EPOCHS} epochs max")
    logger.info(f"Stage 2: Augmentation tuning - {STAGE2_MAX_TRIALS} trials, {STAGE2_MAX_EPOCHS} epochs max")
    ...
    structure_hp_file = f'best_hps_structure_{MODEL_TO_USE}.json'
    ...
    if SKIP_STRUCTURE_TUNING:
        ...
        logger.info("Running structure tuning...")

        tuner_structure = kt.Hyperband(
            ...
            directory='kerastuner_efficient',
            project_name=f'{MODEL_TO_USE}_stage1_structure',
            overwrite=True
        )
        ...
        tuner_structure.search(
            ...
            callbacks=[EarlyStopping(monitor='val_task_a_output_accuracy', mode='max', patience=3, restore_best_weights=True)]
        )
    ...
    augmentation_hp_file = f'best_hps_augmentation_{MODEL_TO_USE}.json'
    ...
    if SKIP_AUGMENTATION_TUNING:
        ...
        logger.info("Running augmentation tuning...")

        tuner_augmentation = kt.Hyperband(
            ...
            hyperband_iterations=1,
            directory='kerastuner_efficient',
            project_name=f'{MODEL_TO_USE}_stage2_augmentation',
            overwrite=True
        )
    ...
    print("\n" + "="*60)
    print(f"EFFICIENT TUNING + FINE-TUNING COMPLETE for {MODEL_TO_USE}")
    print("="*60)
    ...
    # モデル保存
    model_filename = f'best_efficient_tuned_model_{MODEL_TO_USE}.keras'
