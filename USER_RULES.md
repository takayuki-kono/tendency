...
- [ ] 3. コード修正（不要ならスキップ）
- [ ] 4. 修正したPyファイルがあれば `python -m py_compile` で検査（エラー0）
- [ ] 5. リポジトリを変更した場合のみ `git add` / `git commit --author="..." -m "..."` を提示

1. **Prompt (CRITICAL & APPEND ONLY)**
   - ユーザーから受けた新しい依頼内容は、いかなる解析やコード修正の作業の**一番最初**に `docs/prompt.md` へ追記すること。
   - **実装・リポジトリ変更がないターン（議論・質問・方針メモのみ）でも**、同じ2行テンプレで末尾追記する。**対応:** 行は「コード変更なし。」のあとに議論の要点・結論を1行でよい（将来の参照用）。
