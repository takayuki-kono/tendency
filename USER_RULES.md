...
1. **Prompt (CRITICAL & APPEND ONLY)**
   ...
   - **フォーマット固定（2行テンプレ）**:
     ...
     - **YYYY-MM-DD**: "ユーザー発話そのまま（要約/改変禁止）"
       - 対応: 変更点（ファイル名/関数/要点）
     ```
   - **日付の整合（厳守）**: `- **YYYY-MM-DD**:` は**取り違え禁止**。(1) **本日の依頼**は、ユーザー情報の「Today's date」またはユーザーが発話で明示した日付のみを使う。(2) **過去の出来事・既存エントリの訂正**は、可能な限り該当変更の**実コミット日**と一致させる（迷ったら対象ファイルで `git log -1 --format=%ai -- <path>` を参照）。(3) 本文にログのタイムスタンプ（例: `2026-04-22 10:23:17`）を引用する場合は、ヘッダ日付をその**暦日**に合わせ、コミット日と矛盾させない。(4) 並べ替え・一括日付修正も同様に検証する。
