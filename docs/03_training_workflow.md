...
3.  **アンダーサンプリング**:
    - **第 1 段**: フィルタ前の全グループ件数で「2 番目に多いグループ」までをグローバル上限とし、判定後に各グループへ適用（詳細は下記 preprocess_multitask の項）。
    - **第 2 段**（multitask 前処理のみ）: フィルタ＋第 1 段適用後の残件で、**クラス単位**（ラベル先頭セグメント）に「2 番目に多い人」まで再間引き。
...
- **Step 3.5: FT LR Calibration** — `init_weights_path` に Step 3.9 の保存ファイルを設定。保存に成功している場合は `warmup_lr=0` / `warmup_epochs=0` として warmup フェーズをスキップし、head carryover された初期重みから直接 FT に入る（保存失敗時のみ従来の warmup にフォールバック）。終了時の採用スコア（`FINAL_VAL_ACCURACY`）は **FT 開始前の head-only フェーズで観測した検証精度の最大値**（`run_trial` / head LR `run_calibration_trial` / Step 3.9 のいずれかで記録された `FINAL_VAL` の最大）と比較され、**FT キャリブがそれより厳密に低い**（`score_3_5 < head_only_phase_best`）場合は以降 **head-only 仕上げ**に切り替わる（下記）。
- **分岐: FT キャリブが head-only フェーズベスト未満のとき** — **Step 4 / 4.5 / Step 4.7 をスキップ**。`fine_tune=False`・`learning_rate=head_lr`・`init_weights_path=best_head.weights.h5`（無ければ WARNING）で **Best-of-N のみ**（各 run に `--export_model_path=outputs/models/model_seed{seed}.keras`）。採用 seed のモデルを `best_sequential_model.keras` にコピー。`outputs/best_train_params.json` に `finish_mode=head_only`、`score_step_3_9_head` / `score_step_3_5_ft_calib` / **`score_head_only_phase_best`** を記録。
