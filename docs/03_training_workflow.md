...
- **計算式** (2026-02-18更新):
  - `exponent = 1.0` (固定)
  - `adjusted_lr = base_lr * (relative_ratio ** exponent)` (線形スケーリング)
- **Base LR決定ロジック (2026-02-18変更)**:
  - 従来: Target Epoch (10) に収束するLRを探索。
  - **現在**: **Validation Score 最大化** を最優先指標とする。
    1. **Epoch 11** に収束するLR (`lr_11`) を特定。
    2. **Epoch 20** に収束するLR (`lr_20`) を特定。
    3. `[min(lr_11, lr_20), max(lr_11, lr_20)]` の範囲内で、スコアが最大になるLRを二分探索で決定する。
    4. 同点のスコアがある場合も、その範囲を含めて探索を継続し、局所解を逃さないようにする。
- **Early Stoppingの扱い**:
  - LR探索時（キャリブレーション、指数探索ともに）は **有効 (True)**。
  - ただし、学習率の減衰（Decay）が開始された後のみ有効となるよう制御される。
  - **キャリブレーション設定**:
    - 指数探索範囲: `0.15` ～ `1.5`
    - 初期探索点: `[0.15, 0.8, 1.5]`
