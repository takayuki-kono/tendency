...
- **Step 1: Base LR Calibration** — `target_best_epoch=13` を狙う head LR を、**`LRCALIB_BASE_BACKBONE` 固定**でキャリブ。**`initial_lr`** は `lr_calib_context` / 同一ラン直前キャリブとの一致で決定（不一致なら 0.01）。バックボーン切替は 1.1 以降。
