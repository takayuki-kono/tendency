...
- **評価モード（2026-05-18）**: 既定は **`OPTIMIZE_EVAL_USE_FINE_TUNE=True`**（`components/lr_adjustment.py`）。`run_trial` / Step 0 キャリブは **`fine_tune=True`**・`unfreeze_layers=60`（warmup なし）で **FT 本番に近い代理 Val**。旧 head-only 評価に戻す場合は同定数を `False`。`filter_opt_cache` の試行キーに `_evalft=60` / `_evalhead` を付与（モード変更で旧キャッシュと混ざらない）。
...
- **軸探索の評価モード（2026-05-19）**: 既定は **`TRAIN_OPT_EVAL_USE_FINE_TUNE=True`**（`components/lr_adjustment.py`）。**Step 1.1〜3.8** の `run_trial` は **`fine_tune=True`**・`unfreeze_layers=60`（warmup なし）で **FT 本番に近い代理 Val**（`optimize_sequential` と同趣旨）。`train_opt_cache` のキーに `_evalft=60` / `_evalhead` を付与。`False` で従来 head-only。
