...
- **評価モード（2026-05-19）**: 既定は **`OPTIMIZE_EVAL_USE_FINE_TUNE=False`**（head-only）。`True` にすると `run_trial` / Step 0 が **`fine_tune=True`**・`unfreeze_layers=60`（warmup なし）。`filter_opt_cache` の試行キーに `_evalft=60` / `_evalhead` を付与（モード変更で旧キャッシュと混ざらない）。
- **特徴**: SVMよりも最終的なタスクに近いが、1試行に時間がかかる（数分/回、FT 化でさらに増えうる）。
- **探索設定**:
...
- **Step 0（モデル＋FT/head LR）**:
    - `MODEL_NAME_CANDIDATES`（`model_architecture.py`）の**各** `model_name` に対し、フィルタなし前処理の上で **`calibrate_base_lr`（`target_best_epoch=13` 他、他軸と同ロジック）**を実行し、キャリブ終了時の Val スコアが最大のモデルを採択。評価が FT のとき `lr_calib_context.mode=ft`、head-only のとき `mode=head`。
...
- **Base LR決定ロジック**:
  ...
  - **FT の `run_calibration_trial`（`train_sequential`）**: `learning_rate` だけを試行ごとに差し替える。**`warmup_lr` は元 params のまま残すと**ウォームアップが ~head 確定時の大きい LR・本番 FT が試行の小さい LRになり、ログ上「逆転」に見える。**キャリブでは `warmup_lr=0`（ウォームアップ無し）**に揃え、観測を当該試行 LR の単一フェーズに一致させる（`optimize_sequential` の Step0 キャリブも FT 既定）。
- **キャリブレーション設定**:
...
- **軸探索の評価モード（2026-05-19）**: 既定は **`TRAIN_OPT_EVAL_USE_FINE_TUNE=False`**（**Step 1.1〜3.8** は head-only）。`True` で FT 評価。`train_opt_cache` のキーに `_evalft=60` / `_evalhead` を付与。
- **Step 1.1: Model Architecture** — 毎回: `MODEL_NAME_CANDIDATES`（`model_architecture.py`、例: EfficientNet B0/S、ResNet50/101V2、MobileNetV3Large）から `model_name` を比較確定。**MobileNetV4** 相当は **MobileNetV3Large** を代用。`optimize_param` に **`head_only_tie_log` を渡す**（名称は歴史的）。各候補の `run_trial` は `(model, data_file_count, head|ft)` に応じた開始 LR を `resolve_calib_initial_lr` で決め、**LR sweep 内で最良 Val だった `train_lr` をその候補の記憶 LR**として保持する。採択モデルについてはその **記憶 LR（`clip_learning_rate_for_training` 後）を `learning_rate` / `lr_calib_context` の `base_lr` に採用**（FT 評価時は `mode=ft`）。旧 **Step 1.2（採択後の `calibrate_base_lr`）は廃止**（1.1 との LR 探索重複を避ける）。**キャッシュ**は `train_opt_cache` の各キーについて `[score, memorized_lr]` 形式（従来の数値のみの値は旧形式として解釈し、記憶 LR 欠損時は `initial_lr_for_calibrate` にフォールバック）。
- **Step 1.25: Optimizer** — **`model_name` と軸探索 LR（1.1 記憶値）確定後**に、`optimizer` を `optimize_param(['adam','adamw','sgd'])` で比較（評価は上記 FT 既定）。**アーキテクチャや LR が未確定の段階で先に決めることはしない**。
...
- **Step 3.5: FT vs head-only LR Calibration** — **(C) head-only**: `fine_tune=False`・Step 3.9 の `best_head.weights.h5` を `init_weights_path`（無ければ WARNING で空）・`warmup` なしで `calibrate_base_lr` → **`score_step_3_5_head_calib`** / **`lr_step_3_5_head_calib`**。**(A)(B) FT**: 従来どおり carryover（3.9 重み・warmup スキップ）と warmup（`head_lr`・5 epoch）の 2 系統キャリブし高い方を FT 代表（**`score_step_3_5_ft_calib`**）。**`head_finish`**: FT 代表が **(C) head-only 未満**のとき Step 4–4.7 をスキップし head-only Best-of-N（LR は **(C) のキャリブ LR**）。それ以外は FT パイプライン継続。JSON に `score_step_3_5_ft_calib_carry` / `warmup`・`ft_calib_carryover_selected` 等も記録。
- **分岐: `head_finish`（Step 3.5 で head-only が FT 代表以上）** — **Step 4 / 4.5 / Step 4.7 をスキップ**。`fine_tune=False`・`learning_rate=lr_step_3_5_head_calib`・`init_weights_path=best_head.weights.h5` で **Best-of-N のみ**。`finish_mode=head_only` 等を `best_train_params.json` に保存。
