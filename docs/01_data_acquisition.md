...
-   **収集戦略**: 
    -   キーワードは **指定の人物名のみ** を使用（検索クエリに「女優」「顔」などの固定語を自動付与しない）。
...
-   **解像度正規化（類似判別用）**: 解像度差があると類似判定が鈍るため、**最小 face_size**（ファイル名の `sz*`）に合わせて各画像を縮小した**複製**を一時作成し、その複製のみで InsightFace embedding 取得・DBSCAN による類似判別を行う。クラスタ内では解像度の低い方の**元画像**を削除し、処理後に**複製画像はすべて削除**する。
...
## 変更履歴
- **2026-01-27 (2)**: 収集対象のキーワードリストを更新（28名の女優リストを設定）。
