...
# 階層的クラスタリングとユークリッド距離を使用する場合の距離のしきい値。
# この値はデータセットに大きく依存するため、結果を見ながら調整が必要です。
# 一般的にユークリッド距離では0.4 (緩い) ~ 0.6 (厳しい) の範囲で調整します。
# `linkage` パラメータによっても最適な値は変わります。
tolerance = 0.5 # distance_threshold
padding_ratio = 0.0      # 画像の端を拡張する割合 (例: 0.2 = 上下左右に20%ずつ余白を追加)
output_dir = "grouped_faces_cnn_hierarchical" # 分類結果の出力先フォルダ
...
for i, filename in enumerate(image_files):
    ...
    try:
        ...
        # CUDAが有効なdlibがインストールされている場合、GPUを使用して高速化できます。
        face_locations = face_recognition.face_locations(image, model="cnn") # パディングされた画像を使用
...
# 距離計算手法にユークリッド距離を使用し、階層的クラスタリングを実行します。
# n_clusters=None と distance_threshold を設定することで、距離に基づいたクラスタリングを行います。
# linkage='complete' は、クラスタ間の最も遠い要素間の距離を使用します。
...
for label, path in zip(labels, file_paths):
    # 階層的クラスタリングでは全ての点がクラスタに割り当てられます
...
num_clusters = len(set(labels)) # 全ての点がクラスタに属するため、-1の考慮は不要
...
def display_grouped_images_cv(labels, file_paths):
    ...
    for label, path in zip(labels, file_paths):
        # 階層的クラスタリングでは全ての点がクラスタに割り当てられるため、-1のチェックは不要
