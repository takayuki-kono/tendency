...
print("\n顔特徴のクラスタリングを実行します (DBSCAN with Euclidean)...")
encodings_np = np.array(encodings)
# ★★★★★ 変更点: 距離計算手法をユークリッド距離に設定 ★★★★★
...
num_unclassified = list(labels).count(-1)
print(f"\nグループ分け完了: {num_clusters} 人物グループに分類されました。")
print(f"未分類 (ノイズ) の顔: {num_unclassified} 個")
