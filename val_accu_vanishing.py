...
# 設定
ROTATION_RANGE = 10
BATCH_SIZE = 16  # 高速化のため増加
img_size = 112
NUM_SPLITS = 20  # 繰り返し回数を削減（20→10）
EPOCHS_PER_SPLIT = 10  # エポック数を削減（20→10）
...
def analyze_train_contributions_to_val_accu(train_dir, validation_generator, img_size, num_splits=NUM_SPLITS, epochs=EPOCHS_PER_SPLIT, top_n=50):
    """
    訓練データの各画像がval_accuに寄与する度合いを、ランダム分割で分析。
    
    Args:
        train_dir: 訓練データのディレクトリ
        validation_generator: 検証データのジェネレーター
        img_size: 画像のサイズ
        num_splits: 分割の繰り返し回数
        epochs: 各訓練のエポック数
        top_n: 表示する画像の数
    """
    # 訓練データのファイルリストを取得
    ...
    for split_idx in range(num_splits):
        ...
        for temp_dir, paths in [(temp_dir_a, group_a_paths), (temp_dir_b, group_b_paths)]:
            ...
            for path in paths:
                cat = 'category1' if 'category1' in path else 'category2'
    ...
    # トップ10を表示
    print(f"\nTop {top_n} training images with lowest contribution to val_accu:")
    plt.figure(figsize=(15, 5))
    for i, contrib in enumerate(contributions[:top_n]):
        img_path = contrib['path']
        category = contrib['category']
        val_accu_sum = contrib['val_accu_sum']
        count = contrib['count']
        ...
        plt.title(f"Sum: {val_accu_sum:.4f}\nCat: {category}\nCount: {count}")
        plt.axis('off')
        
        print(f"Image {i+1}: {img_path}")
        print(f"  Category: {category}, val_accu_sum: {val_accu_sum:.4f}, Count: {count}")
    ...
    df.to_csv('train_val_accu_contributions.csv', index=False)
    print("Results saved to 'train_val_accu_contributions.csv'")
